...
[![Four Modes](https://img.shields.io/badge/Modes-4-d75d32?style=flat-square)](#four-outputs-one-black-field-breakout-logic)
...
022 reverses that logic:

```text
lock source facts → choose one horizontal or vertical pure-black rectangle → place most of the subject inside it → let exactly one defining feature break out → rebuild the subject with clean stable elastic black line and white negative form → extract one source colour and illuminate exactly one small detail → finish with microtype along the edge, negative space, or breakout direction
```
...
- **Exactly one breakout feature:** the most recognisable head contour, branch, roof, limb, or object edge emerges from, passes through, grows out of, or breaks the boundary.
- **Clean elastic hand line:** black line is concise, smooth, stable, and naturally elastic; reject jitter, repeated tracing, hairy noise, and sterile machine-uniform vector outlines.
- **White negative plus tiny grey:** white negative form carries recognition and grey occupies only very small planes; never turn the subject into a pure black silhouette.
...
The repository reserves [`assets/examples/`](assets/examples/) for future work. Only finished 022 artwork verified by the project owner will be added; no post or image from another style is used as a placeholder.

Future samples will demonstrate 022's adaptability. Their subjects, metaphors, palette, copy, and canvas ratios will never become generation references or defaults.

## Four outputs, one black-field breakout logic
...
| --- | --- | --- |
| `top-bottom` | source-adaptive | complete source above, 022 black-field breakout line design below; both panels retain the source size and split exactly 50/50 |
| `left-right` | source-adaptive | complete source left, 022 black-field breakout line design right; both panels retain the source size and split exactly 50/50 |
...
Exact user pixels > explicit ratio or destination > source adaptation for ordinary modes. The original `022.md` used a 3:4 creative canvas, but that historical example is not a silent default in the current Skill.
...
The image model creates the source-bound black field, subject occupancy, exactly one breakout feature, clean stable hand line, white negative form, tiny grey planes, exactly one source-derived colour signal, and editorial type. `scripts/compose_panel.py` only plans canvases, performs exact 50/50 raster composition, finalises dimensions, and audits results. It never fakes artwork with programmatic drawing.
...
- [Chinese full prompt](references/xxd-panel-022-prompt.zh-CN.md)
- [English full prompt](references/xxd-panel-022-prompt.en.md)
- [Original style brief](references/022-source.md)
...
**Let one feature cross the boundary, and let only one colour turn on.**
